...
export const getAllReviewsAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("https://grocery-store-a5j0.onrender.com/api/user/get/reviews");
...
export const deleteReviewsAction = (reviewId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`https://grocery-store-a5j0.onrender.com/api/user/admin/review/${reviewId}`);
