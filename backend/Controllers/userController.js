...
const userLogin = async (req, res) => {
  try {
    ...
    if (email && password) {
      const isUserExit = await userModel.findOne({ email: email });
      if (isUserExit) {
        const isPassMatch = await bcryptjs.compare(password, isUserExit.password);
    ...
    if (email) {
      ...
      if (isUserExit) {
        const token = jwt.sign(
          ...
          JWT_RESET_PASSWORD_SECRET_KEY='YouAreUnknown',
          {
            expiresIn: "5m",
          }
        );
        ...
        const transporter = nodemailer.createTransport({
          service: "gmail",
          auth: {
            user: SMPT_MAIL='geo.farrell46@ethereal.email',
            pass: SMPT_PASSWORD='hJmQV5NUHG7rJ1Vu55',
          },
        });
        ...
        await transporter.sendMail({
          from: SMPT_MAIL='geo.farrell46@ethereal.email',
          to: isUserExit.email,
...
const userPasswordReset = async (req, res) => {
  try {
    ...
    await jwt.verify(token, JWT_RESET_PASSWORD_SECRET_KEY='YouAreUnknown');
    if (password && confirm_password) {
      if (password === confirm_password) {
        if (password.length >= 4) {
